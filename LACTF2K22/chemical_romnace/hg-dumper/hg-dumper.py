...
def fetch_hg(url, directory, jobs, retry, timeout):
    ...
    if (
        ...
        and "requires" in get_indexed_files(response)
    ):
    ...
    process_tasks(tasks, DownloadWorker, jobs, args=(url, directory, retry, timeout))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-j", "--jobs", type=int, default=10, help="number of simultaneous requests"
    )
    parser.add_argument(
        "-r",
        "--retry",
