...
if (location.pathname !== '/') {
  ...
  xhr.addEventListener("load", () => {
    if (xhr.status === 200) {
      ...
      // Parsing json here so we can have prototype pollution
      const json = JSON.parse(xhr.response);

      for (const key of Object.keys(json)) {
        if (interestObj[key] === undefined) {
          // Assigning to interestObj[key] directly is safe because as far as ik 
          // there is no way to set __proto__ on interestObj
          interestObj[key] = json[key];
        } else{
          // but here we are assigning json[key] to interestObj[key] which is not safe, let me try it 
          Object.assign(interestObj[key], json[key]);
          

        }
      }
      console.log( "Am i polluted", interestObj.polluted);
    } else {
      alert(xhr.response);
      location.href = '/';
    }
