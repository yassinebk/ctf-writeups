...
def threadTransact():
    ...
    while True:
        if QUEUE:
            ...
            if p.requests > 10:
                print('Hit limit:',datetime.now().isoformat(), key)
                #Throttling, ignore request and attempts to restore

                # Disaster as trading doesn't invlove this xd
                # So we can basically hit the limit + 1 and then for each request we reset the old portflio stonsk
                if key in bkup:
                    p = DB.getInstance().getUser(key)
...
def market_scrape():
    while True:
        if TRADEPOST:
            ...
            print("Verified stonk with key:",key)
