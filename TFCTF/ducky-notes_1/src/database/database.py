...
def db_register(username, password):
    con = sqlite3.connect("database/data.db")
    user = query(
        con, "SELECT username FROM users WHERE username = ?", (username,), True
    )
...
def db_create_post(username, post):
    ...
    query(
        con,
        "INSERT into posts (user_id, title, content, hidden) VALUES (?, ?, ?, ?)",
        (
            user["id"],
            post["title"],
            post["content"],
...
def db_get_user_posts(username, hidden):
    ...
    posts = query(
        con,
        "SELECT users.username as username, title, content, hidden from posts INNER JOIN users ON users.id = posts.user_id WHERE users.username = ?",
        (username,),
    )
...
def db_get_all_users_posts():
    ...
    posts = query(
        con,
        "SELECT users.username as username, title, content, hidden from posts INNER JOIN users ON users.id = posts.user_id ",
    )
    return posts
