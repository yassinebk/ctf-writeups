    ...
    return db.getUser(name).then(async (user) => {
      if (user.used === 1) {
        return res.render("coupon", {
          msg: "Already used coupon.",
          name: name,
        });
      }
      if (user.coupon !== coupon) {
        return res.render("coupon", {
          msg: "Does not match the coupon you have.",
          name: name,
        });
      }
  ...
  router.post("/register", auth, async (req, res) => {
    ...
    try {
      let response = await axios.get(
        `http://${HOST}:${PORT}/verify?coupon=${coupon}`,
      );
      const { result, coupon_data } = response.data;

      if (result) {
        if (coupon_data.admin === "1" && parseInt(coupon_data.value) <= 2000) {
          return db
            .addCoupon(
              coupon,
              coupon_data.name,
  ...
  router.post("/welcome", auth, async (req, res) => {
    ...
    try {
      ...
      response = await axios.get(
        `http://${HOST}:${PORT}/get_coupon?key=${key.msg}`,
      );
