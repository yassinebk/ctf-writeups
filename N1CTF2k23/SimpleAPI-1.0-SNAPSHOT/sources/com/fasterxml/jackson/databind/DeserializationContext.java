...
/* loaded from: SimpleAPI-1.0-SNAPSHOT.jar:BOOT-INF/lib/jackson-databind-2.11.0.jar:com/fasterxml/jackson/databind/DeserializationContext.class */
public abstract class DeserializationContext extends DatabindContext implements Serializable {
    ...
    public <T> T readValue(JsonParser p, JavaType type) throws IOException {
        ...
        if (deser == null) {
            reportBadDefinition(type, "Could not find JsonDeserializer for type " + ClassUtil.getTypeDescription(type));
        }
    ...
    public Object handleUnexpectedToken(Class<?> instClass, JsonParser p) throws IOException {
        return handleUnexpectedToken(constructType(instClass), p.getCurrentToken(), p, (String) null, new Object[0]);
    }
    ...
    public Object handleUnexpectedToken(JavaType targetType, JsonParser p) throws IOException {
        return handleUnexpectedToken(targetType, p.getCurrentToken(), p, (String) null, new Object[0]);
    }
        ...
        while (true) {
            ...
            if (h != null) {
                ...
                if (instance != DeserializationProblemHandler.NOT_HANDLED) {
                    ...
                    reportBadDefinition(targetType, String.format("DeserializationProblemHandler.handleUnexpectedToken() for type %s returned value of type %s", ClassUtil.getClassDescription(targetType), ClassUtil.classNameOf(instance)));
                }
                problemHandlers = h.next();
            } else {
                if (msg2 == null) {
                    if (t == null) {
                        msg2 = String.format("Unexpected end-of-input when binding data into %s", ClassUtil.getTypeDescription(targetType));
                    } else {
                        msg2 = String.format("Cannot deserialize instance of %s out of %s token", ClassUtil.getTypeDescription(targetType), t);
                    }
                }
