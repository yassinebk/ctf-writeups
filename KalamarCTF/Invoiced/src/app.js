...
app.post('/checkout', async (req, res) => {
 ...
 app.get("/pdf", async (req, res) => {
  //TODO: add order to database
  let pdffile = await pdf.renderSimplePdf(req.query.url)
  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader('Content-Disposition', 'inline; filename=invoice.pdf')
  return res.send(pdffile)
})  
...
app.get('/orders', (req, res) => {
  ...
  if (req.socket.remoteAddress != "::ffff:127.0.0.1") {
    return res.send("Nice try, ip bad")
  }
  if (req.cookies['bot']) {
    console.log('cookies problem',req.cookies['bot'])
    return res.send("Nice try, cookie not set")
  }
